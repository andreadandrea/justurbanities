...
0. **Accounts (NEW, ratified 2026-07-07 — supersedes spec §2.5):** SupabaseAuth (plain fetch) + AccountPanel 👤 (register/login) + PlayerDashboard 📊 (my game / my contribution) + CloudSaves cross-device resume shipped. **User actions pending:** run `docs/mp/player_saves.sql` in the SQL editor; in Authentication → Sign In / Up decide "Confirm email" (off = classroom-friendly). Follow-ups: tighten session_events RLS to authenticated ids, facilitator ownership of sessions, privacy notice for minors (GDPR — emails are now personal data).
