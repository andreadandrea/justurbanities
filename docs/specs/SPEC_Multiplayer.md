...
5. **Identity & privacy (GDPR-light):** no accounts, no email. Pseudonymous display name + device key per session; 6-char session codes; sessions expire (default 30 days) and are erasable by the facilitator in one action. No personal data in events.
